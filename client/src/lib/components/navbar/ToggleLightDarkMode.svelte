...
<div class="flex items-center justify-center p-4">
	<button
		class="relative flex items-center p-1 transition-colors duration-300 ease-in-out bg-gray-700 rounded-full w-14 h-7 dim:bg-gray-800"
		on:click={toggleTheme}
		aria-label="Toggle theme"
	>
		<div
			class="absolute w-5 h-5 duration-300 ease-in-out transform bg-white rounded-full shadow-md toggle-dot"
			class:translate-x-7={theme === 'dim'}
		></div>
		<div
		...
		>
			{#if theme === 'light'}
				<div class="text-white dim:text-black moon-icon" />
			{:else}
				<div class="sun-icon" />
			{/if}
		</div>
